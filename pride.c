...
PRIDEDEF PrideContext pride_create_context(u32 *pixels, size_t width, size_t height, size_t stride);
// PRIDEDEF PrideContext pride_subcontext(PrideContext pc, int x, int y, int w, int h);
...
// PRIDEDEF void pride_line(PrideContext pc, int x1, int y1, int x2, int y2, u32 color);
PRIDEDEF b32 pride_normalize_triangle(size_t width, size_t height, int x1, int y1, int x2, int y2, int x3, int y3, int *lx, int *hx, int *ly, int *hy);
PRIDEDEF b32 pride_barycentric(int x1, int y1, int x2, int y2, int x3, int y3, int xp, int yp, int *u1, int *u2, int *det);
// PRIDEDEF void pride_triangle(PrideContext pc, int x1, int y1, int x2, int y2, int x3, int y3, u32 color);
PRIDEDEF void pride_triangle3c(PrideContext pc, int x1, int y1, int x2, int y2, int x3, int y3, u32 c1, u32 c2, u32 c3);
// PRIDEDEF void pride_triangle3z(PrideContext pc, int x1, int y1, int x2, int y2, int x3, int y3, f32 z1, f32 z2, f32 z3);
// PRIDEDEF void pride_triangle3uv(PrideContext pc, int x1, int y1, int x2, int y2, int x3, int y3, f32 tx1, f32 ty1, f32 tx2, f32 ty2, f32 tx3, f32 ty3, f32 z1, f32 z2, f32 z3, PrideContext texture);
// PRIDEDEF void pride_triangle3uv_bilinear(PrideContext pc, int x1, int y1, int x2, int y2, int x3, int y3, f32 tx1, f32 ty1, f32 tx2, f32 ty2, f32 tx3, f32 ty3, f32 z1, f32 z2, f32 z3, PrideContext texture);
...
#define PRIDE_RGBA(r, g, b, a) ((((r)&0xFF)<<(8*0)) | (((g)&0xFF)<<(8*1)) | (((b)&0xFF)<<(8*2)) | (((a)&0xFF)<<(8*3)))
...
PRIDEDEF b32 pride_barycentric(int x1, int y1, int x2, int y2, int x3, int y3, int xp, int yp, int *u1, int *u2, int *det) {
  ...
  *u2   = ((y3 - y1)*(xp - x3) + (x1 - x3)*(yp - y3));  
  ...
  return (
    (PRIDE_SIGN(int, *u1) == PRIDE_SIGN(int, *det) || *u1 == 0) &&
...
PRIDEDEF void pride_triangle3c(PrideContext pc, int x1, int y1, int x2, int y2, int x3, int y3, u32 c1, u32 c2, u32 c3) {
  int lx, hx, ly, hy;
  if (pride_normalize_triangle(pc.width, pc.height, x1, y1, x2, y2, x3, y3, &lx, &hx, &ly, &hy)) {
    for (int y = ly; y <= hy; y++) {
      for (int x = lx; x <= hx; x++) {
        int u1, u2, det;
        if (pride_barycentric(x1, y1, x2, y2, x3, y3, x, y, &u1, &u2, &det)) {
          pride_blend_color(&PRIDE_PIXEL(pc, x, y), mix_colors3(c1, c2, c3, u1, u2, det));
        }
      }
    }
  }
